...
- **Domain packages are pure**: production code in `internal/domain/*` imports stdlib, sibling domain packages, and shopspring/decimal only. Test files may add test-only dependencies (e.g. pgregory.net/rapid); `_test` packages never enter a consumer's build graph.
